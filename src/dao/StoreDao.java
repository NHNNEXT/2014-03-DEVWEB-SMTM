...
public class StoreDao {
	public int insertStore(final Store store) {
		...
		return template.executeUpdate(sql, store.getUserSeq(), store.getName(),
				store.getAddr(), store.getPhone());
	}

	public int insertEmpt(final Employment empt) {
		JdbcTemplate template = new JdbcTemplate();
		String sql = "INSERT INTO EMPLOYMENT(EMPT_STO_SEQ, EMPT_USER_SEQ) VALUES(?,?)";
		return template.executeUpdate(sql, empt.getStoreSeq(),
				empt.getUserSeq());
	}
	...
	public ArrayList<Store> retrieveStoreForEmpt(final String storeId,
	...
	private RowMapper<Store> resultSetOfStore() {
		return new RowMapper<Store>() {
			public Store mapRows(ResultSet rs) throws SQLException {
				return new Store(rs.getString("STO_SEQ"),
						rs.getString("STO_USER_SEQ"), rs.getString("STO_NAME"),
						rs.getString("STO_ADDR"), rs.getString("STO_PHONE"),
